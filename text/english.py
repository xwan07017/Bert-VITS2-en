...
import shutil
cmudict_dir = './venv/nltk_data/corpora'
av_p_t_dit = './venv/nltk_data/taggers'
if not os.path.exists(cmudict_dir):
    os.mkdir(cmudict_dir)
if not os.path.exists(av_p_t_dit):
    os.mkdir(av_p_t_dit)
shutil.copy("./cmudict.zip", cmudict_dir+"/cmudict.zip")
shutil.copy("./averaged_perceptron_tagger.zip", av_p_t_dit+"/averaged_perceptron_tagger.zip")
_g2p = G2p()
